...
const APP_AUTOSAVE_INTERVAL_DEFAULT_VALUE = 60000;
...
function autosave() {
  ...
  notes_instance.length().then(function (length) {
    if (length <= 0 || !length) notes_instance.setItem("1", new notes(markdown_text_box.innerText));else notes_instance.getItem(Number(length).toString()).then(function (value) {
      if (value.date.toDateString() === new Date().toDateString()) notes_instance.setItem(Number(length).toString(), new notes(markdown_text_box.innerText));else notes_instance.setItem(Number(length + 1).toString(), new notes(markdown_text_box.innerText));
    });
  });
}

localforage.getItem(APP_AUTOSAVE_SETTING).then(function (value) {
  if (!value) setInterval(autosave, APP_AUTOSAVE_INTERVAL_DEFAULT_VALUE);else setInterval(autosave, value);
...
notes_instance.length().then(function (length) {
  ...
  if (!length) return;
  notes_instance.getItem(Number(length).toString()).then(function (note_object) {
    if (!note_object) return;else {
  ...
  notes_instance.getItem(Number(length - 1).toString()).then(function (note_object) {
    const current_date = new Date();
    current_date.setDate(current_date.getDate() - 1);
    if (!note_object) localforage.setItem(APP_NOTE_DAY_STREAK, 0);else if (current_date.toDateString() === note_object.date.toDateString()) localforage.getItem(APP_NOTE_DAY_STREAK).then(days => {
      localforage.setItem(APP_NOTE_DAY_STREAK, days++);
...
function convert_to_regex_string(string) {
  ...
  for (let char = 0, length = string.length; char < length; char++) {
    if (regex_compliant_characters.includes(string[char])) regex_string += `\\${string[char]}`;else regex_string += string[char];
  }
...
function surround_text(text_string, surrounding_string) {
  ...
  if (regex_match) return text_string.replace(regex_match[1], regex_match[2]);else {
